...
conf = {
    ...
    'N_GAMES': 50,  # Number of games of self play generated by best_model, 25k in paper
    'MCTS_SIMULATIONS': 10, # 1.6k in paper
    'SIZE': 9, # board size 19 in paper
    'STOP_EXPLORATION': 30, # Number of plays after which temperature goes to 0 , 30 in paper

    'TRAIN_BATCH_SIZE': 2048,  # Batch size in the training phase, 32 in paper
    'EPOCHS_PER_SAVE': 10, # A model will be saved to be evaluated this amount of epochs, 1000 in paper
    'NUM_WORKERS': 1,# We use this many GPU workers so split the task, 64 in paper
    'L2_EPSILON': 1e-4,# The epsilon coefficient in the loss value function, 1e-4 in paper

    'EVALUATE_N_GAMES': 40,# The number of games to test on to elect new best model, 400 in paper
